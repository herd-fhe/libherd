...
TEST(XlssTranslator, translate_simple_function)
{
	...
	ASSERT_EQ(stage_circuit.inputs.size(), 1);
	ASSERT_EQ(stage_circuit.inputs[0].size(), 1);
	EXPECT_EQ(stage_circuit.inputs[0][0], herd::common::DataType::INT8);
...
TEST(XlssTranslator, translate_simple_reduce_function)
{
	herd::utils::TempStorage storage;
	...
	std::string sample = "using int8_t=char;\n"
						 ...
						 "};\n"
						 "#pragma hls_top\n"
						 "out_output top_add3(in_agg agg, in_input input)\n"
						 "{\n"
						 "    return {input.a | 0x01, agg.b | 0b01010101};\n"
						 "}";
	herd::translator::xlscc::detail::Compiler translator(storage);
	...
	EXPECT_EQ(circuit.source_nodes().size(), 18);

	ASSERT_EQ(stage_circuit.inputs.size(), 2);
	ASSERT_EQ(stage_circuit.inputs[0].size(), 1);
	EXPECT_EQ(stage_circuit.inputs[0][0], herd::common::DataType::INT8);
	ASSERT_EQ(stage_circuit.inputs[1].size(), 1);
	EXPECT_EQ(stage_circuit.inputs[1][0], herd::common::DataType::INT8);
