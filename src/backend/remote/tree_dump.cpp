...
#include "herd/translator/common/tree/node/operation_node.hpp"
#include "herd/translator/common/tree/visitor.hpp"


namespace herd
{
	proto::CircuitGraph dump_tree(const translator::CircuitGraph& graph)
	{
		...
		std::unordered_map<std::shared_ptr<translator::AbstractNode>, uint32_t> id_map;
		uint32_t next_id = 0;
		...
		const auto assign_ids = [&next_id, &id_map](std::shared_ptr<translator::AbstractNode>& node) {
			if(node->children().empty() || node->parents().empty())
			{
				return translator::VisitStatus::CONTINUE;
			}
		...
		const auto assign_children = [&operation_nodes, &id_map](std::shared_ptr<translator::AbstractNode>& node) {
			if(node->children().empty() || node->parents().empty())
			{
				return translator::VisitStatus::CONTINUE;
			}

			const auto operation_node = std::dynamic_pointer_cast<translator::OperationNode>(node);
		...
		translator::visit(graph, assign_ids);
		translator::visit(graph, assign_children);
